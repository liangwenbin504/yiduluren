...
if sizhu_data.get('success'):
    ...
    # 比用法
    print(f"\n比用法:")
    xia_zei_shang = [r for r in ke_results if r[0] == '贼']
    shang_ke_xia = [r for r in ke_results if r[0] == '克']
    
    print(f"  下贼上: {xia_zei_shang}")
    print(f"  上克下: {shang_ke_xia}")
    
    # 判断阳日阴日
    YANG_RI = ['甲', '丙', '戊', '庚', '壬']
    is_yang_ri = day_gan in YANG_RI
    print(f"  日干: {day_gan}({'阳' if is_yang_ri else '阴'}日)")
    
    if xia_zei_shang:
        print(f"  有下贼上，优先处理下贼上")
        if len(xia_zei_shang) == 1:
            chu_chuan = xia_zei_shang[0][1]
            print(f"  只有一个下贼上，初传: {chu_chuan}")
        else:
            print(f"  有多个下贼上，需要比用")
            YANG_ZHI = ['子', '寅', '辰', '午', '申', '戌']
            bi_yong_results = []
            for r in xia_zei_shang:
                shang = r[1]
                if is_yang_ri and shang in YANG_ZHI:
                    bi_yong_results.append(r)
                elif not is_yang_ri and shang not in YANG_ZHI:
                    bi_yong_results.append(r)
            
            print(f"  比用结果: {bi_yong_results}")
            
            if len(bi_yong_results) == 1:
                chu_chuan = bi_yong_results[0][1]
                print(f"  比用后只有一个，初传: {chu_chuan}")
            else:
                print(f"  比用后仍有多个，需要涉害法")
    else:
        print(f"  无下贼上，处理上克下")
        if len(shang_ke_xia) == 1:
            chu_chuan = shang_ke_xia[0][1]
            print(f"  只有一个上克下，初传: {chu_chuan}")
        else:
            print(f"  有多个上克下，需要比用")
            YANG_ZHI = ['子', '寅', '辰', '午', '申', '戌']
            bi_yong_results = []
            for r in shang_ke_xia:
                shang = r[1]
                if is_yang_ri and shang in YANG_ZHI:
                    bi_yong_results.append(r)
                elif not is_yang_ri and shang not in YANG_ZHI:
