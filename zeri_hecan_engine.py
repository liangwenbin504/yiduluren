...
def zeri_signals(keti: str, zetiri_type: str, sizhu: dict = None, shan_wx: str = '',
    ...
    for k in ('斗首_四柱全廉', '斗首_年柱破鬼', '斗首_月柱武财', '斗首_日时柱元辰武财',
              ...
              '斗首_月建元辰', '斗首_月建武财', '斗首_月建廉子', '斗首_月建贪官',
    ...
    try:
        ...
        sig['斗首_破鬼月'] = _s1 == '破鬼'
        # 月建到山吉凶（正文第三章五"斗首二十四山每年月建到山吉凶"：
        # "元辰、武才、廉子作月建吉，贪官、破鬼作月建凶，俱管一年"）
        sig['斗首_月建元辰'] = _s1 == '元辰'
        sig['斗首_月建武财'] = _s1 == '武财'
        sig['斗首_月建廉子'] = _s1 == '廉贞'
        sig['斗首_月建贪官'] = _s1 == '贪官'
        ...
        _sl = _shan_lu(shan)
        if _nian_zhi:
            # 山禄贵_同太岁：只取山自身禄贵（干山贵/支山山支 + 山禄），排除"太岁支=向支"
            # 平凡情形（2026-08-21 精化：岁君临向另判 太岁_临向，语义分离提高区分度）
            _gui_shan = _shan_xiang_gui(shan, '')
            sig['山禄贵_同太岁'] = _nian_zhi in (_gui_shan | ({_sl} if _sl else set()))
            # 太岁_临向：岁君到向（书"末传子竟作太岁加在丁未向上…其福力又加倍已"）
            sig['太岁_临向'] = bool(_xiang) and _nian_zhi == _xiang
        if _sc and _gui_sx:
...
_RULE_LABELS = {
    ...
    'ZY-047': '廉子出外死',
    'ZY-048': '全联元曜', 'ZY-049': '三元一武', 'ZY-050': '三元一廉', 'ZY-051': '二元二武',
    'ZY-052': '两元夹一廉', 'ZY-053': '岁日夹战', 'ZY-054': '破鬼月', 'ZY-055': '贪破居六相',
    'ZY-056': '凶宜在外', 'ZY-057': '元辰居墓绝', 'ZY-058': '财马贴元辰', 'ZY-059': '元武逢岁驾',
    'ZY-060': '文昌入课', 'ZY-061': '天喜入课', 'ZY-062': '拱贵', 'ZY-063': '建贵',
    'ZY-064': '禄马贵三秀', 'ZY-065': '岁君临向',
    'ZY-066': '月建元辰', 'ZY-067': '月建武财', 'ZY-068': '月建廉子', 'ZY-069': '月建贪官',
}
