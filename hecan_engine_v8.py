...
def load_rules(path: str = '') -> list:
    p = path or os.path.join(_ROOT, 'data', 'hecan_rules_v3.json')
    try:
...
def build_signals(ri_gan, ri_zhi, sanchuan, tianjiang_list, kong, sike, keti,
                  zhanlei, wangshuai_fn=None, yuejiang='', tianjiang_map=None) -> dict:
    ...
    # BUG-B 修复 2026-08-19：干上神所乘天将应按天将映射（原实现误要求干上神=初传）
    _tjm = tianjiang_map or {}
    if _tjm and gan_shang:
        sig['干上_乘白虎'] = _tjm.get(gan_shang) == '白虎'
        sig['干上_乘青龙'] = _tjm.get(gan_shang) == '青龙'
    else:
