...
const LunarDate = {
    // 农历数据（1900-2100年）
    // 数据格式：每行代表一年，每月用十六进制表示
    // 高位表示月份大小（1=30天，0=29天），低位表示闰月月份
    LUNAR_INFO: [
    ...
    lYearDays(y) {
        ...
        for (i = 0x8000; i > 0x8; i >>= 1) {
            sum += (this.LUNAR_INFO[y - 1900] & i) ? 1 : 0;
        }
    ...
    leapDays(y) {
        if (this.leapMonth(y)) {
            return (this.LUNAR_INFO[y - 1900] & 0x10000) ? 30 : 29;
        }
        return 0;
    },
    ...
    solar2lunar(y, m, d) {
        ...
        return {
            ...
            lDay: day,
            isLeap: isLeap,
            monthCn: (isLeap ? '闰' : '') + this.LUNAR_MONTH[month - 1] + '月',
            dayCn: this.LUNAR_DAY[day - 1],
            yearCn: this.getYearCn(year),
            shengxiao: this.SHENGXIAO[(year - 4) % 12]
        };
    },
