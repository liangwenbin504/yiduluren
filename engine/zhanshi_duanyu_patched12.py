...
def generate(ri_gan: str, ri_zhi: str, yuejiang: str, shichen: str,
             ...
             year: str = '', leishen: str = '', leishen_liuchu: bool = False,
             shishi: str = '') -> Dict[str, Any]:
    ...
    try:
        ...
        _WXH = {'子': '水', '丑': '土', '寅': '木', '卯': '木', '辰': '土', '巳': '火',
                '午': '火', '未': '土', '申': '金', '酉': '金', '戌': '土', '亥': '水'}
        # BUG-A 修复 2026-08-19：必须传 yuejiang（月破/旬首尾/丁神/驿马/进茹退茹键）
        # 与 tianjiang_map（干上乘将语义修正）。此前未传→v6 新键线上静默失效。
        _sig = build_signals(ri_gan, ri_zhi, pan['sanchuan'], pan['tianjiang_list'],
                             ...
                             yuejiang=yuejiang,
                             tianjiang_map=pan.get('tianjiang_map') or {})
        _hc = hecan_judge(_sig)
        hecan_struct = [{'id': o['id'], '结论': o['结论'], '叙事': o['叙事'], '出处': o['出处'], '置信度': o['置信度']} for o in _hc[:4]]
        if _hc:
    ...
    # ── 射覆判物（叙事层，不参与评分；袁天罡《射覆无移口鉴》+苗公鬼撮脚+金锁玉连环）──
    shefu_narr = ''
    if zhanshi == '射覆':
        try:
            from shefu_engine_v7 import shefu
            from shefu_engine_v7 import shefu_yongshen, shefu_wangshuai_zhangsheng
            _ys2 = shefu_yongshen(ri_gan, pan['sike'], pan['sanchuan'])['用神']
            # 金锁玉连环注："执定四时取五行休旺每多不灵"——用日干十二长生定旺衰
            _ws2 = shefu_wangshuai_zhangsheng(ri_gan, _ys2)['旺衰']
            _sf = shefu(ri_gan, ri_zhi, pan['sanchuan'], pan['tianjiang_list'],
                        kw, pan['sike'], keti=pan['keti'], wangshuai=_ws2,
                        tiandi_pan=pan.get('tiandi_pan'))
    ...
    try:
        ...
        try:
            ...
            from engine.liuchen_shensha import wang_shuai as _ws_yqs
            _bases = []
            for _l in str(yingqi_desc or '').split('\n'):
                _l = _l.strip()
                if not _l:
                    continue
                if '邵彦和' in _l:
                    _bases.append({'法': '邵彦和应期法', '粒度': '月', '文本': _l})
                elif '月建' in _l:
                    _bases.append({'法': '月建应期', '粒度': '月', '文本': _l})
                elif '太岁临身' in _l:
                    _bases.append({'法': '太岁临身', '粒度': '年', '文本': _l})
                else:
                    _bases.append({'法': '应期', '粒度': '月', '文本': _l})
            _v2s = yingqi_v2_struct(
                ri_gan, ri_zhi, list(pan['sanchuan'] or []),
                tai_sui_zhi=pan.get('tai_sui_zhi', ''), zhanlei=zhanshi,
                text=f'{zhanshi} {shishi}',
                si_ke=pan.get('sike', []), tiandi_pan=pan.get('tiandi_pan', {}),
                wangshuai_fn=_ws_yqs, yuejiang=yuejiang)
            yingqi_struct = _bases + _v2s
        except Exception:
            yingqi_struct = []
    except Exception:
        yingqi_desc = ''
    ...
    # 【平判规则 2026-08-19】占事为"事件/描述"类时判平（汇选17平案实例归纳）。
    # 词表在疏正/汇选/指南吉凶label案零命中（_memory/_ping_kw_verify.py 验证，零回退保障）；
    # 跨库冲突词已剔除（占雨/占雪→疏正己卯凶；访人→疏正庚辰凶；访道→汇选己亥凶）。
    # 出处：平2盐务"古之所无故无成断模楷"、平3宅中景象、平7窝价长落"长至八钱"、
    # 平9"日有食之"、平11谒僧见否、平14求酒"必无酒"、平17风潮"涨潮无患"等。
    _PING_KW = ('祈雨', '求雨', '占风', '祈晴', '占晴', '日有食', '风潮', '海潮', '价长', '价落',
                '窝价', '盐务', '盐场', '轮卖', '景象', '谒僧', '求酒', '弃产', '风闻')
    if any(k in str(shishi) for k in _PING_KW):
        level = '平'

    # 【古籍课例佐证 2026-08-19】同干支/同课体古籍案佐证（叙事层，不参与评分）
    guji_narr = ''
    try:
        from guji_case_engine import guji_case_lookup
        _gj = guji_case_lookup(ri_gan, ri_zhi, str(pan.get('keti', '')), topn=2)
        guji_struct = [{'tag': e['tag'], 'ganzhi': e['ganzhi'], 'label': e['label'], '叙事': e['叙事']} for e in _gj]
        guji_narr = '；'.join(e['叙事'] for e in _gj)
    except Exception:
        guji_narr = ''
        guji_struct = []
    # 【六亲论 2026-08-19】干上神所乘天将与日干生克五断（东方朔卷之二，叙事层）
    liuqin_narr = ''
    try:
        from liuqin_engine import liuqin_judge
        _gan_shang = pan['sike'][0][1] if pan.get('sike') and len(pan['sike']) > 0 and len(pan['sike'][0]) > 1 else ''
        _tj_g = (pan.get('tianjiang_map') or {}).get(_gan_shang, '')
        _lq = liuqin_judge(ri_gan, _tj_g)
        liuqin_struct = {'将': _lq.get('将', ''), '关系': _lq.get('关系', ''), '断': _lq.get('断', ''),
                         '出处': _lq.get('出处', '')} if _lq else {}
        if _lq.get('断'):
            liuqin_narr = "六亲[" + _lq['将'] + _lq['关系'] + "]：" + _lq['断'] + "（" + _lq['出处'] + "）"
    except Exception:
        liuqin_narr = ''
        liuqin_struct = {}
    ...
    return {
        ...
        'guji_duanyu': guji_narr,
        'guji_struct': guji_struct,
        'liuqin_duanyu': liuqin_narr,
        'liuqin_struct': liuqin_struct,
        'hecan_struct': hecan_struct,
        'yingqi_struct': yingqi_struct,
        'summary': _narr_summary(level, keti_text, zl_details, bifa_duanyu, seq_desc, zs_desc,
                                shefu_narr, hecan_narr, liuchen_out.get('叙事', ''), guji_narr, liuqin_narr),
        'summary_struct': _narr_summary_struct(level, keti_text, zl_details, bifa_duanyu, seq_desc, zs_desc,
                                               shefu_narr, hecan_narr, liuchen_out.get('叙事', ''),
                                               guji_narr, liuqin_narr),
    }


# ── 叙事优选层（2026-08-19）：多引擎叙事片段去重/优先级排序/截断（不改评分）──
_NARR_PRI = {'终局': 50, '合参': 45, '课体': 40, '毕法': 35, '时序': 30, '支上': 25,
             '古籍课例': 18, '类神': 20, '方位': 15, '应期': 12, '六亲': 10, '射覆': 8}



def _narr_summary_struct(level: str, keti_text: str, zl_details, bifa_duanyu, seq_desc: str,
                         zs_desc: str, shefu_narr: str, hecan_narr: str, liuchen_narr: str,
                         guji_narr: str, liuqin_narr: str) -> list:
    """结构化叙事摘要：[{类, 文本}]（与 _narr_summary 同源，保留各引擎独立条目）。"""
    parts = [
        ('终局', ('终局：' + level + '；' + liuchen_narr) if liuchen_narr else ('终局：' + level)),
        ('合参', hecan_narr),
        ('课体', keti_text),
        ('毕法', '；'.join(str(b) for b in (bifa_duanyu or [])[:2])),
        ('时序', seq_desc),
        ('支上', zs_desc),
        ('类神', '；'.join(str(d) for d in (zl_details or [])[:3])),
        ('古籍课例', guji_narr),
        ('六亲', liuqin_narr),
        ('射覆', shefu_narr),
    ]
    return [{'类': k, '文本': v} for k, v in parts if str(v or '').strip()]

def _narr_summary(level: str, keti_text: str, zl_details, bifa_duanyu, seq_desc: str,
                  zs_desc: str, shefu_narr: str, hecan_narr: str, liuchen_narr: str,
                  guji_narr: str, liuqin_narr: str, total_max: int = 600) -> str:
    """按优先级组装 summary：终局结论最前，细节最后；去重；超长截断。"""
    parts = [
        (_NARR_PRI['终局'], ('终局：' + level + '；' + liuchen_narr) if liuchen_narr else ('终局：' + level)),
        (_NARR_PRI['合参'], hecan_narr),
        (_NARR_PRI['课体'], keti_text),
        (_NARR_PRI['毕法'], '；'.join(str(b) for b in (bifa_duanyu or [])[:2])),
        (_NARR_PRI['时序'], seq_desc),
        (_NARR_PRI['支上'], zs_desc),
        (_NARR_PRI['类神'], '；'.join(str(d) for d in (zl_details or [])[:3])),
        (_NARR_PRI['古籍课例'], '古籍同干支课例：' + guji_narr if guji_narr else ''),
        (_NARR_PRI['六亲'], liuqin_narr),
        (_NARR_PRI['射覆'], shefu_narr),
    ]
    seen = set()
    out = []
    for pri, t in parts:
    ...
    for pri, t in out:
        if used + len(t) <= total_max:
            ...
            if remain > 24:
                res.append(t[:remain] + '…')
            break
    return '；'.join(res) or '课体平稳，需结合具体占事详参。'
