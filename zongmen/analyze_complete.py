...
def main():
    ...
    # 查找所有"第 X 课"的行（可能后面有冒号或文字）
    print("\n查找课的开始位置...")
    lesson_starts = []
    
    for i, line in enumerate(lines):
        # 匹配"第一课"、"第二课"等（前后可能有空格）
        line_stripped = line.strip()
        match = re.match(r'^(第 [一二三四五六七八九十百千\d]+课)', line_stripped)
        if match:
            lesson_starts.append((i, match.group(1)))
            if len(lesson_starts) <= 10:
                print(f"  行{i}: {match.group(1)}")
    ...
    for idx, (line_num, lesson_line) in enumerate(lesson_starts[:64]):
        ...
        if ':' in current_line:
            title = current_line.split(':')[0].replace(lesson_number, '').strip()
        else:
    ...
    json_path = os.path.join(OUTPUT_DIR, '64_lessons_complete.json')
    ...
    # 生成 Markdown 表格
    md_path = os.path.join(OUTPUT_DIR, '64_lessons_complete_table.md')
