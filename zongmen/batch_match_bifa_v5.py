...
def generate_duan_yu(matched_rules: List[Dict]) -> str:
    ...
    for rule in sorted_rules[:5]:  # 只取前 5 条规则生成断语
...
def batch_match_bifa_v5():
    """批量匹配毕法赋 v5.0（100 条规则，性能优化）"""
    print("="*80)
    print("  毕法赋规则匹配系统 v5.0")
    print("  第五阶段：100 条规则，性能优化，断语生成")
    print("="*80)
    print()
    ...
    print("正在并行匹配毕法赋规则（100 条）...")
    start_time = time.time()
    
    bifa_results = {}
    processed = 0
    
    # 使用线程池并行处理（8 个线程，v5 优化）
    with ThreadPoolExecutor(max_workers=8) as executor:
        ...
        for future in as_completed(futures):
            try:
                ...
                # 添加断语生成
                result['duan_yu'] = generate_duan_yu(result['bifa_matched_rules'])
    ...
    print("正在保存匹配结果...")
    try:
        with open('bifa_matched_results_v5.json', 'w', encoding='utf-8') as f:
            json.dump(bifa_results, f, ensure_ascii=False, indent=2)
        print("  JSON 结果已保存：bifa_matched_results_v5.json")
    except Exception as e:
    ...
    try:
        ...
        df.to_excel('bifa_matched_results_v5.xlsx', index=False)
        print("  Excel 报表已保存：bifa_matched_results_v5.xlsx")
    except Exception as e:
