...
if 'access_token' not in token_result:
    print("X 获取 token 失败：" + str(token_result))
    exit(1)
...
for name, base_url in endpoints:
    ...
    if 'error_code' in result:
        print(f"  X 错误 {result['error_code']}: {result['error_msg']}")
    else:
        print(f"  OK 成功！识别行数：{len(result.get('words_result', []))}")
        if result.get('words_result'):
            first_line = result['words_result'][0].get('words', '')
            print(f"     前 3 行：{first_line[:30]}...")
