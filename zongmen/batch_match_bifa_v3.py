...
from bifa_rules_engine_v3 import BiFaRulesMatcherV3
from daliuren_base import enhance_ke_li_data

def batch_match_bifa_v3():
    """批量匹配毕法赋规则 v3"""
    print("="*70)
    print("  《毕法赋》规则批量匹配程序 v3.0")
    print("="*70)
    print()
    ...
    for key, ke_li in matched_results.items():
        try:
            # 增强课例数据（添加天将、神煞等）
            ke_li_enhanced = enhance_ke_li_data(ke_li, year=2024)
            ...
            bifa_results[key] = {
                ...
                'chu_sang_men': ke_li_enhanced.get('chu_sang_men', False),
                'chu_diao_ke': ke_li_enhanced.get('chu_diao_ke', False),
            }
    ...
    output_json = {
        ...
        'match_method': 'bifa_rules_v3',
        'rule_statistics': rule_stats,
        'matched_results': bifa_results
    }
    
    json_file = r'd:\新建文件夹\仪度六壬择日\yiduluren\zongmen\bifa_matched_results_v3.json'
    ...
    for key, result in bifa_results.items():
        ...
        row = {
            ...
            '丧门': result.get('sang_men', ''),
            '吊客': result.get('diao_ke', ''),
            '病符': result.get('bing_fu', ''),
    ...
    excel_file = r'd:\新建文件夹\仪度六壬择日\yiduluren\zongmen\bifa_matched_results_v3.xlsx'
    df.to_excel(excel_file, index=False, engine='openpyxl')
    
    print(f"✓ Excel 报告已保存")
    
    print("\n" + "="*70)
    print("  ✅ 毕法赋批量匹配 v3.0 完成！")
    print("="*70)
    print(f"\n新增功能：")
    print(f"  ✓ 集成丧门、吊客、病符等神煞")
    print(f"  ✓ 规则扩展至 20+ 条")
    print(f"  ✓ 输出包含年神煞信息")
    print()
