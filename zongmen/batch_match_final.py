...
PROGRESS_FILE = r'd:\新建文件夹\仪度六壬择日\yiduluren\zongmen\match_progress_final.json'
RESULT_FILE = r'd:\新建文件夹\仪度六壬择日\yiduluren\zongmen\qwen_max_matched_results_final.json'
        ...
        if response.status_code == 200:
            ...
            if json_match:
                ...
                if matched and matched[0] in ke_jing_data:
                    ...
                    return None, f"课经无效：{matched[0] if matched else '空'}"
            else:
...
def batch_match():
    ...
    for i, (ke_li_key, ke_li) in enumerate(ke_li_data.items()):
        ...
        print(f"[{i+1}/{total}] {ke_li_key} | 成功:{success_count} 失败:{error_count} | 预计剩余:{int(remaining/60)}分{int(remaining%60)}秒", end=" ")
