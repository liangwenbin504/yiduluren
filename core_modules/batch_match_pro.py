...
def batch_match_with_pro():
    """使用专业版课体判断批量匹配"""
    print("=" * 80)
    print("使用专业课体判断模块批量匹配课例")
    print("=" * 80)
    ...
    ke_ti_judge_pro = KeTiJudgeProfessional()
    ...
    for ke_key, ke_data in ke_li_data.items():
        ...
        try:
            ...
            ke_ti_list = ke_ti_judge_pro.judge_ke_ti(
                sike, sanchuan, tiandi_pan,
                ri_gan, ri_zhi, ri_gan_zhi,
                yue=yue, shi=shi,
                lunar_month=1, nian_zhi=yue
            )
    ...
    # 保存更新后的数据
    output_file = 'data/720_ke_li_matched_pro.json'
    ...
    print(f"\n64 课经匹配情况:")
    print(f"  已匹配课体：{len(matched_ke_ti_names)}/64")
    print(f"  匹配率：{len(matched_ke_ti_names)/64*100:.1f}%")
